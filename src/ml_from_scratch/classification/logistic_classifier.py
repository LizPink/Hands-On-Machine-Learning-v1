...
class LogisticClassifier:
    ...
    def load_train_and_valid(self, X_train, y_train, X_valid, y_valid):
        ...
        self.y_valid = y_valid.reshape((-1,1))
        self.theta = np.random.normal(size=X_train.shape[1]).reshape((-1,1))
    ...
    def predict_proba(self, X_test:np.ndarray):
        return self.logistic( X_test @ self.theta )

    def predict(self, X_test:np.ndarray, threshold:float):
